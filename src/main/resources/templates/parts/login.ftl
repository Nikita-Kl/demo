...
<#macro login path isRegisterForm>
    <form action="${path}" method="post">
        ...
        <#if isRegisterForm>
            ...
            <div class="col-sm-6">
                <div class="g-recaptcha" data-sitekey="6LcIhj4lAAAAAIYRApr9nJO8Ebe2Ybnc8B6S8zNZ"></div>
                <#if captchaError??>
...
<#macro logout>
    <form action="/logout" method="post">
        <input type="hidden" name="_csrf" value="${_csrf.token}" />
        <<button class="btn btn-primary btn-exit" type="submit"><#if user??>Выйти<#else>Войти</#if></button>
    </form>
</#macro>